...
class MotorControl:
    ...
    def recv(self):
        ...
        try:
            ...
            if message:
                self.__process_can_message(message)
    ...
    def recv_set_param_data(self):
        ...
        try:
            message = self.can_bus.recv(timeout=0.1)
            if message:
                self.__process_set_param_can_message(message)
        except Exception as e:
            pass  # Timeout or other error

    def __process_can_message(self, message):
        """
        处理接收到的CAN消息
        """
    ...
    def __process_set_param_can_message(self, message):
        """
        处理设置参数的CAN消息
        """
        CANID = message.arbitration_id
