...
from spacemouse_ros import Spacemouse
...
def print_pose(translation, rotation_rpy):
    # 把4*4矩阵转为xyz, 欧拉角
...
class TeleopNode(Node):
    def __init__(
        ...
        self.eef_traj_pub = self.create_publisher(EEFTraj, "go2_arm/eef_traj", 10)
        self.eef_state_sub = self.create_subscription(
            EEFState, "go2_arm/eef_state", self.eef_state_callback, 10
        )
    ...
    def ctrl_callback(self):
        if not self.started:
            ...
            # print_pose(spacemouse_state[:3], spacemouse_state[3:])
            button_left = self.spacemouse.is_button_pressed(0)
            button_right = self.spacemouse.is_button_pressed(1)
