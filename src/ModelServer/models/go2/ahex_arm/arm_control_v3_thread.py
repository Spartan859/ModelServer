...
class AhexArm:
    ...
    def __init__(self, control_frequency=50.0):
        """
        ...
        # Differential control parameters - maximum magnitudes
        self.__p_max = 1.0  # maximum position change per step
        self.__v_max = 1.0  # maximum velocity 
        self.__a_max = 1.0  # maximum acceleration
        self.__j_max = 1.0  # maximum jerk (third derivative)
        
        # Control gains for differential equations
        self.__k_p = 1.0 / (1.0 / self.__control_frequency)  # position gain
        self.__k_v = 1.0 / (1.0 / self.__control_frequency)  # velocity gain  
        self.__k_a = 1.0 / (1.0 / self.__control_frequency)  # acceleration gain
        
        # Control state variables - current velocity, acceleration for each joint
        self.__control_state_lock = threading.Lock()
        self.__current_velocity = np.zeros(self.__joint_num)
        self.__current_acceleration = np.zeros(self.__joint_num)
    ...
    def _compute_differential_control(self, current_pos, target_pos, dt):
        """
        Compute control position using differential equations with constraints
        
        Based on the differential control algorithm:
        1. Calculate position error: e_k = x_{d,k} - x_k
        2. Calculate desired velocity: v_d = sat(k_p * e_k, v_max)  
        3. Calculate desired acceleration: a_d = sat(k_v * (v_d - v_k), a_max)
        4. Calculate jerk: j_k = sat(k_a * (a_d - a_k), j_max)
        5. Update acceleration: a_{k+1} = sat(a_k + j_k * dt, a_max)
        6. Update velocity: v_{k+1} = sat(v_k + a_k * dt + 0.5 * j_k * (dt)^2, v_max)
        7. Update position: x_{k+1} = sat(x_k + v_k * dt + 0.5 * a_k * (dt)^2 + (1/6) * j_k * (dt)^3, p_max)
        ...
        with self.__control_state_lock:
            current_vel = self.__current_velocity.copy()
            current_acc = self.__current_acceleration.copy()
        
        # Step 1: Calculate position error
        e_k = target_pos - current_pos
        
        # Step 2: Calculate desired velocity
        v_d = self._sat(self.__k_p * e_k, self.__v_max)
        
        # Step 3: Calculate desired acceleration  
        a_d = self._sat(self.__k_v * (v_d - current_vel), self.__a_max)
        
        # Step 4: Calculate jerk
        j_k = self._sat(self.__k_a * (a_d - current_acc), self.__j_max)
        
        # Step 5: Update acceleration
        new_acc = self._sat(current_acc + j_k * dt, self.__a_max)
        
        # Step 6: Update velocity
        new_vel = self._sat(current_vel + current_acc * dt + 0.5 * j_k * (dt**2), self.__v_max)
        
        # Step 7: Update position
        control_pos = self._sat(
            current_pos + current_vel * dt + 0.5 * current_acc * (dt**2) + (1.0/6.0) * j_k * (dt**3), 
            self.__p_max
        )
        
        # Update internal state variables
        with self.__control_state_lock:
            self.__current_velocity[:] = new_vel
            self.__current_acceleration[:] = new_acc
    ...
    def __control_thread_func(self):
        ...
        while not self.__stop_event.is_set() and not rospy.is_shutdown():
            ...
            control_pos = self._compute_differential_control(current_pos, target_pos, dt)
            
            # Apply deadzone for first 3 joints (optional, can be removed if not needed)
            for i in range(3):
    ...
    def set_target_position(self, target_pos, timeout=10.0):
        ...
        # Update shared target position - the differential control algorithm
        # will naturally adjust velocity and acceleration for smooth transition
        with self.__target_lock:
    ...
    def set_target_position_smooth(self, target_pos, total_time=10.0, time_step=None):
        """Set target position with smooth non-linear trajectory
        
        使用平滑的非线性轨迹将机械臂从当前位置移动到目标位置。
        这个方法会逐步更新目标位置，配合差分控制算法实现平滑运动。
        注意：差分控制算法会自然地处理速度和加速度的连续性，无需重置状态。
        
        Args:
            target_pos: 目标关节位置 (6个元素)
            total_time: 总运动时间，单位秒 (默认: 10.0)
            time_step: 时间步长，单位秒 (默认: None，自动根据控制频率计算)
        ...
        # 如果没有指定time_step，则根据控制频率自动计算
        if time_step is None:
            time_step = 1.0 / self.__control_frequency
    ...
    def set_control_frequency(self, frequency):
        ...
        # Update gains based on new frequency
        self.__k_p = 1.0 / (1.0 / frequency)
        self.__k_v = 1.0 / (1.0 / frequency)  
        self.__k_a = 1.0 / (1.0 / frequency)
    
    def set_differential_limits(self, p_max=None, v_max=None, a_max=None, j_max=None):
        """Set maximum limits for differential control
        
        Args:
            p_max (float): Maximum position change per step
        ...
        if a_max is not None and a_max > 0:
            self.__a_max = a_max
        if j_max is not None and j_max > 0:
            self.__j_max = j_max
    
    def set_control_gains(self, k_p=None, k_v=None, k_a=None):
        """Set control gains for differential equations
        
        Args:
            k_p (float): Position gain
            k_v (float): Velocity gain  
            k_a (float): Acceleration gain
        """
        if k_p is not None and k_p > 0:
            self.__k_p = k_p
        if k_v is not None and k_v > 0:
            self.__k_v = k_v
        if k_a is not None and k_a > 0:
            self.__k_a = k_a
    
    def get_differential_limits(self):
        """Get current differential control limits
        
        Returns:
            dict: Dictionary containing p_max, v_max, a_max, j_max
        """
        return {
            'p_max': self.__p_max,
            'v_max': self.__v_max,
    ...
    def get_control_gains(self):
        ...
        Returns:
            dict: Dictionary containing k_p, k_v, k_a
        """
        return {
            'k_p': self.__k_p,
            'k_v': self.__k_v,
    ...
    def get_current_control_state(self):
        """Get current velocity and acceleration states
        
        Returns:
            dict: Dictionary containing current velocity and acceleration
        """
        with self.__control_state_lock:
            return {
                'velocity': self.__current_velocity.copy(),
                'acceleration': self.__current_acceleration.copy()
            }
